...
import {
  ...
  DEFAULT_CATCH_ERR,
  INVALID_QUERY_ERR,
} from 'src/ado/types/ado.constants'
import { queryMsgs } from 'src/ado/types/ado.querymsg'
...
export class AuctionService extends AdoService {
  ...
  public async getLatestAuctionState(
    ...
    try {
      const queryMsgStr = JSON.stringify(queryMsgs.auction.latest_auction_state)
  ...
  public async getAuctionState(address: string, auctionId: number): Promise<AuctionStateResponse> {
    try {
      const queryMsgStr = JSON.stringify(queryMsgs.auction.auction_state).replace(
        AUCTION_QUERY_AUCTION_ID,
        String(auctionId),
      )
  ...
  public async getBids(address: string, auctionId: number): Promise<BidsResponse> {
    try {
      const queryMsgStr = JSON.stringify(queryMsgs.auction.bids).replace(AUCTION_QUERY_AUCTION_ID, String(auctionId))
  ...
  public async getAuctionIDs(address: string, tokenId: string, tokenAddress: string): Promise<AuctionIDsResponse> {
    try {
      const queryMsgStr = JSON.stringify(queryMsgs.auction.auction_ids)
        .replace(AUCTION_QUERY_TOKEN_ID, tokenId)
        .replace(AUCTION_QUERY_TOKEN_ADDRESS, tokenAddress)
  ...
  public async getAuctionInfosForAddress(
    ...
    try {
      const queryMsgStr = JSON.stringify(queryMsgs.auction.auction_infos_for_address).replace(
        AUCTION_QUERY_TOKEN_ADDRESS,
        tokenAddress,
      )
